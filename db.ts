...
// وظيفة للمساعدة في مراقبة الحالة
const logStatus = (msg: string, data?: any) => {
  console.log(`[Google Sync] ${msg}`, data || '');
};
...
export const syncToCloud = async (email: string, data: any): Promise<void> => {
  logStatus("بدء محاولة المزامنة...");
  
  if (!window.gapi) {
    logStatus("⚠️ خطأ: مكتبة gapi غير محملة! تأكد من ملف index.html");
    return;
  }

  if (!window.gapi.client?.drive) {
    logStatus("⚠️ خطأ: Drive API غير مفعل أو لم يتم تحميله بعد.");
    return;
  }
  ...
  try {
    ...
    logStatus("✅ تم رفع البيانات بنجاح إلى درايف!");
  } catch (e) {
    logStatus("❌ فشلت المزامنة:", e);
  }
};

export const fetchFromCloud = async (email: string): Promise<any | null> => {
  logStatus("محاولة جلب البيانات من السحاب...");
  ...
  try {
    ...
    const fileId = listResponse.result.files?.[0]?.id;
    if (!fileId) {
      logStatus("ℹ️ لم يتم العثور على ملفات سحابية سابقة.");
      return null;
    }
    ...
    logStatus("✅ تم جلب البيانات بنجاح من درايف.");
    return response.result;
  } catch (e) {
    logStatus("❌ فشل جلب البيانات:", e);
    return null;
  }
};

// ... (احتفظ ببقية الوظائف الأصلية كما هي) ...
